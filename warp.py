...
with open (data_csv) as target:
    ...
    for row in reader:
        ...
        dz.append(findZ(pos_angle))

warp1 = cv2.imread(data_dir+str(start_no)+'.jpg')
...
roll_bias = roll_targets[start_no]
pitch_bias = -pitch_targets[start_no] 
z_bias = 0#13.5 - dz[start_no]

#H = findHomography(roll_bias, pitch_bias, 0., np.array([[0.], [z_bias], [0.]])) #13.5-z
#warp1 = cv2.warpPerspective(warp1, H, (width, height))

#0617flat: 50, 0.01, 30, 1200
#0617random1: 50, 0.01, 50, 1200
dist_width = 800
dist_height = 640
features_count = 50
features_res = 0.1
...
#for i in range(start_no+1, len(pitch_targets)):
    print(i)
    warp2 = cv2.imread(data_dir + str(i) + '.jpg')
    ...
    window_center, f = gf.matchFeatures(warp2, window_center, f, dist_width, dist_height, features_count, features_res, features_dist, match_minimum_dist,
